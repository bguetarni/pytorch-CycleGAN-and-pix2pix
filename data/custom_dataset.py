    ...
    def __init__(self, opt):
        ...
        for i, s in enumerate(samples):
            ...
            for img in images_list:
                img = cv2.imread(self.root + 'inputs/' + s + '/' + img).astype('float32')
            ...
            y = tmp
            for j in range(n_pano):
                Y[i, :, :, 3*j:3*(j+1)] = y[:, 128*j:128*(j+1), :]
        self.X = torch.Tensor(X.transpose(0, 3, 1, 2))
        self.Y = torch.Tensor(Y.transpose(0, 3, 1, 2))
